...
4. **Install dependencies:**
   ...
   ```powershell
   pip install flask langchain langchain_huggingface langchain_community langchain_openai sentence-transformers faiss-cpu numpy scikit-learn pymupdf langchain-mistralai mistralai
   ```
...
6. **Generate an API key from **[**Mistral AI**](https://mistral.ai/)**:**

   - Sign up / log in.
   - Create a new API key.
   - In `app.py`, find the line around **31**:
     ```python
     os.environ["MISTRAL_API_KEY"] = "FbLAJBFsSnqIjydjm3W2DHfsdzpVJq14"
     ```
     Replace the example key with **your own generated API key**.
...
- **LLM API:**
  ...
  - Mistral AI (model: `mistral-medium`)
...
- **API Key:** Mistral AI (`MISTRAL_API_KEY`)
